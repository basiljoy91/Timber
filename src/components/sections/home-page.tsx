  ...
  return (
    <main className="relative overflow-clip pb-24">
      <header className="section-shell relative z-20 flex items-center justify-between py-6">
        ...
        <nav className="hidden items-center gap-8 rounded-full border border-white/60 bg-white/70 px-6 py-3 text-sm text-soft-ink shadow-[0_10px_28px_rgba(44,31,19,0.06)] backdrop-blur md:flex">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
        ...
        <a
          href="#products"
          className="hidden items-center gap-2 rounded-full bg-ink px-5 py-3 text-sm font-semibold text-white shadow-[0_14px_34px_rgba(23,21,20,0.16)] hover:-translate-y-0.5 hover:bg-[#2a2827] md:inline-flex"
        >
          Shop now
      ...
      <section className="section-shell relative grid items-center gap-14 pb-20 pt-6 lg:grid-cols-[0.92fr_1.08fr] lg:pb-28 lg:pt-10">
        <div className="max-w-2xl">
          ...
          <motion.h1
            className="mt-8 max-w-xl font-display text-5xl leading-[0.95] font-semibold tracking-[-0.05em] text-ink sm:text-6xl lg:text-[5.6rem]"
            initial={{ opacity: 0, y: 34 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.85, delay: 0.12, ease: [0.22, 1, 0.36, 1] }}
          >
            Make your interior feel collected, modern, and unforgettable.
          </motion.h1>

          <motion.p
            className="mt-6 max-w-xl text-lg leading-8 text-soft-ink sm:text-xl"
          ...
          >
            {heroChips.map((chip) => (
              <span key={chip} className="chip text-sm font-medium">
                {chip}
              </span>
            ))}
          </motion.div>
        ...
        <div className="relative h-[700px] lg:h-[760px]">
          <motion.div
            className="glass-panel absolute inset-y-14 left-0 hidden w-[72%] rounded-[40px] lg:block"
            animate={
              prefersReducedMotion
                ? { rotate: 0, y: 0 }
                : { rotate: [6, 3, 6], y: [0, -14, 0] }
            }
            transition={{
              duration: 10,
          ...
          <motion.div
            className="absolute right-0 top-10 w-full max-w-[640px] origin-bottom-left rounded-[42px] border border-white/65 bg-[#17181b] p-6 text-white shadow-[0_40px_110px_rgba(26,18,10,0.22)]"
            initial={{ opacity: 0, x: 46, y: 30, rotate: -2 }}
          ...
          <motion.article
            className="glass-panel absolute right-6 top-0 w-48 rounded-[30px] p-4"
            initial={{ opacity: 0, y: 24, x: 18 }}
            animate={
              prefersReducedMotion
                ? { opacity: 1, y: 0, x: 0 }
                : { opacity: 1, y: [0, -12, 0], x: 0 }
            }
            transition={{
              duration: prefersReducedMotion ? 0.7 : 6.5,
              delay: 0.46,
              repeat: prefersReducedMotion ? 0 : Infinity,
              ease: "easeInOut",
            }}
          ...
          <motion.article
            className="glass-panel absolute -left-2 bottom-16 max-w-[220px] rounded-[30px] p-5"
            initial={{ opacity: 0, y: 28 }}
            animate={
              prefersReducedMotion
                ? { opacity: 1, y: 0 }
            ...
            transition={{
              duration: prefersReducedMotion ? 0.75 : 7.2,
              delay: 0.38,
              repeat: prefersReducedMotion ? 0 : Infinity,
              ease: "easeInOut",
            }}
          ...
          <motion.div
            className="absolute bottom-2 right-10 flex items-center gap-3 rounded-full border border-white/70 bg-white/76 px-5 py-3 text-sm font-medium text-ink shadow-[0_14px_30px_rgba(44,31,19,0.08)] backdrop-blur"
            initial={{ opacity: 0, y: 20 }}
            animate={
              prefersReducedMotion
                ? { opacity: 1, y: 0 }
            ...
            transition={{
              duration: prefersReducedMotion ? 0.7 : 5.4,
              delay: 0.54,
              repeat: prefersReducedMotion ? 0 : Infinity,
              ease: "easeInOut",
            }}
      ...
      <section id="categories" className="section-shell py-14">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-3">
          {featuredCategories.map((collection, index) => (
            <motion.article
